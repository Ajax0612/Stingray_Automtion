...
public class CommonFunction_CCJ extends TestBase{

	SimpleDateFormat df = new SimpleDateFormat();
	Properties CCJ_Rater = new Properties();
	...
	public Map<String,Map<String,Double>> CAN_CCJ_ReturnP_Values_Map = new HashMap<>();
	...
	public void NewBusinessFlow(String code,String event) throws ErrorInTestMethod{
		...
		try{
			...
			if(((String)common.NB_excel_data_map.get("CD_Money&Assault")).equals("Yes")){	
				...
				customAssert.assertTrue(MoneyAssaultPage(common.NB_excel_data_map), "Material DamagePage function is having issue(S) . ");
			}
			...
			if(((String)common.NB_excel_data_map.get("CD_PublicLiability")).equals("Yes")){	
				...
				customAssert.assertTrue(PublicLiabilityPage(common.NB_excel_data_map), "Public Liability function is having issue(S) . ");
			}
			if(((String)common.NB_excel_data_map.get("CD_DeteriorationofStock")).equals("Yes")){	
				...
				customAssert.assertTrue(DeteriorationofStockPage(common.NB_excel_data_map), "Deterioration of Stock function is having issue(S) . ");
			}
			if(((String)common.NB_excel_data_map.get("CD_GoodsInTransit")).equals("Yes")){	
				...
				customAssert.assertTrue(GoodsinTransitPage(common.NB_excel_data_map), "Goods in Transit function is having issue(S) . ");
			}
			if(((String)common.NB_excel_data_map.get("CD_LegalExpenses")).equals("Yes")){	
				...
				customAssert.assertTrue(LegalExpensesPage(common.NB_excel_data_map), "Legal Expenses function is having issue(S) . ");
			}
		...
		try{
			...
			if((internal_data_map.get("Property Details").get(count).get("PoD_AddBuildings")).equalsIgnoreCase("Yes")){
				...
				if(coverBasis.equalsIgnoreCase("Day 1")){
					...
					customAssert.assertTrue(k.Input("CCJ_MD_BD_SumInsured", internal_data_map.get("Property Details").get(count).get("AddBuilding_Suminsured")), "Unable to enter Sum Insured value for buildings.");
				}
				...
				common_CCJ.MD_Building_Occupancies_list.add(occupancy);
			...
			if((internal_data_map.get("Property Details").get(count).get("PoD_AddContents")).equalsIgnoreCase("Yes")){
				...
				customAssert.assertTrue(k.DropDownSelection("CCJ_MD_CO_ItemType", contentType), "Unable to select below ground level dropdown from contents.");
				...
				customAssert.assertTrue(k.DropDownSelection("CCJ_MD_CO_method", coverBasis), "Unable to select below ground level dropdown from contents.");
				...
				if(coverBasis.equalsIgnoreCase("Day 1")){
					...
					customAssert.assertTrue(k.Input("CCJ_MD_CO_SumInsured", internal_data_map.get("Property Details").get(count).get("AddContent_Suminsured")), "Unable to enter Sum Insured value for buildings.");
				...
				customAssert.assertTrue(k.DropDownSelection("CCJ_MD_CO_TeritorialLimit", internal_data_map.get("Property Details").get(count).get("AddContent_TerritorialCover")), "Unable to select below ground level dropdown from contents.");
			...
			if((internal_data_map.get("Property Details").get(count).get("PoD_AddSpecifiedContents")).equalsIgnoreCase("Yes")){
				...
				customAssert.assertTrue(k.DropDownSelection("CCJ_MD_SPCO_method", coverBasis), "Unable to select below ground level dropdown from contents.");
				...
				customAssert.assertTrue(k.DropDownSelection("CCJ_MD_CO_TeritorialLimit", internal_data_map.get("Property Details").get(count).get("AddSPContent_TerritorialCover")), "Unable to select below ground level dropdown from contents.");
...
public boolean MoneyAssaultPage(Map<Object, Object> map_data){
	...
	try{
			...
			customAssert.assertTrue(k.Input("CCJ_MA_MoneyAtHome", (String)map_data.get("MA_MoneyAtHome")), "Unable to enter on Money at home of authorised person  Money & Assualt screen.");
...
public boolean PublicLiabilityPage(Map<Object, Object> map_data){
	...
	try{
			...
			customAssert.assertTrue(k.Input("CCJ_PL_IndemnityLimit", (String)map_data.get("PL_IndemnityLimit")), "Unable to enter Indemnity limit (£) on  Public Liability screen.");
            ...
            for(int count=1;count<=no_of_property;count++){
               ...
               //Referal - Activity as per SUP-748
  	           if(is_PL_referral_activity(activity)){
  	        	   common_HHAZ.referrals_list.add("Refer Activity - "+activity);
  	           } 
...
public boolean EmployersLiabilityPage(Map<Object, Object> map_data){
	...
	try{
			...
			customAssert.assertTrue(k.Input("CCJ_EL_IndemnityLimit", (String)map_data.get("EL_IndemnityLimit")), "Unable to enter Indemnity limit (£) on Employers Liability screen.");
           ...
           // customAssert.assertTrue(k.DropDownSelection("CCD_EL_RIDDORClaims", (String)map_data.get("EL_RIDDORClaims")),"Unable to select RIDDOR Claims in Employers Liability .");
    	        ...
    	        for(int count=1;count<=no_of_property;count++){
    	           ...
    	           // if(k.isDisplayed("//table//td[text()='Employee Wages Breakdown']//following::select[contains(@name,'ccc_el_activities')]")){
    	            //int index = getIndexDropdownBD(internal_data_map.get("Loss Of Rent").get(count-1).get("LOI_CoverName"));
    	            activity = internal_data_map.get("AddEmpWages").get(count-1).get("EL_ACT_Activity").replaceAll("\u00A0", " ").trim();
    	            customAssert.assertTrue(k.DropDownSelection_WebElement(elm.get(count-1), activity),"Unable to select Activities in Public Liability Activities.");
    	           // }
...
public boolean DeteriorationofStockPage(Map<Object, Object> map_data){
	...
	try{
	    ...
	    customAssert.assertTrue(k.Input("CCJ_DOS_SumInsured", (String)map_data.get("DOS_SumInsured")), "Unable to Enter Sum Insured on Deterioration of Stock Page.");
	...
	boolean retValue = true;
		...
		try{
           ...
           for(int count=1;count<=no_of_property;count++){
                ...
                elm = driver.findElements(By.xpath("//*[contains(@name,'p6_ccj_sum_insured-')]"));
            ...
            //BI Extensions
            elm = driver.findElements(By.xpath("//*[contains(@name,'ccj_bi_ext_sum_insured_ro')]"));          
            ...
            no_of_property = properties.length;
             ...
             for(int count=1;count<=no_of_property;count++){
                ...
                elm = driver.findElements(By.xpath("//*[text()='Additional Extensions']//following::*[contains(@name,'ccj_bi_extension')]"));
                ...
                elm = driver.findElements(By.xpath("//*[text()='Additional Extensions']//following::*[contains(@name,'ccj_bi_ext_sum_insured')]"));
...
public void RenewalFlow(String code,String fileName) throws ErrorInTestMethod{
	...
	*/	if(((String)common.Renewal_excel_data_map.get("CD_MaterialDamage")).equals("Yes")){	
			customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Material Damage"),"Issue while Navigating to Material Damage screen.");
			customAssert.assertTrue(MaterialDamagePage(common.Renewal_excel_data_map), "Material DamagePage function is having issue(S) . ");
		}	
		...
		if(((String)common.Renewal_excel_data_map.get("CD_Money&Assault")).equals("Yes")){	
			customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Money & Assault"),"Issue while Navigating to Money & Assault screen.");
			customAssert.assertTrue(MoneyAssaultPage(common.Renewal_excel_data_map), "Material DamagePage function is having issue(S) . ");
		}
		...
		if(((String)common.Renewal_excel_data_map.get("CD_PublicLiability")).equals("Yes")){	
			customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Public Liability"),"Issue while Navigating to Public Liability screen.");
			customAssert.assertTrue(PublicLiabilityPage(common.Renewal_excel_data_map), "Public Liability function is having issue(S) . ");
		}
		if(((String)common.Renewal_excel_data_map.get("CD_PersonalAccidentStandard")).equals("Yes")){	
			...
			customAssert.assertTrue(PersonalAccidentStandardPage(common.Renewal_excel_data_map), "Personal Accident Standard function is having issue(S) . ");
		}
		if(((String)common.Renewal_excel_data_map.get("CD_PersonalAccidentOptional")).equals("Yes")){	
			customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Personal Accident Optional"),"Issue while Navigating to Personal Accident Optional screen.");
			customAssert.assertTrue(PersonalAccidentOptionalPage(common.Renewal_excel_data_map), "Personal Accident Optional function is having issue(S) . ");
		}
		if(((String)common.Renewal_excel_data_map.get("CD_DeteriorationofStock")).equals("Yes")){	
			customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Deterioration of Stock"),"Issue while Navigating to Deterioration of Stock screen.");
			customAssert.assertTrue(DeteriorationofStockPage(common.Renewal_excel_data_map), "Deterioration of Stock function is having issue(S) . ");
		}
		if(((String)common.Renewal_excel_data_map.get("CD_GoodsInTransit")).equals("Yes")){	
			customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Goods in Transit"),"Issue while Navigating to Goods in Transit screen.");
			customAssert.assertTrue(GoodsinTransitPage(common.Renewal_excel_data_map), "Goods in Transit function is having issue(S) . ");
		}
		if(((String)common.Renewal_excel_data_map.get("CD_LegalExpenses")).equals("Yes")){	
			customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Legal Expenses"),"Issue while Navigating to Legal Expenses screen.");
			customAssert.assertTrue(LegalExpensesPage(common.Renewal_excel_data_map), "Legal Expenses function is having issue(S) . ");
		}
...
//*************************START***********************************//
	...
	public double get_Book_Rate_from_Properties(String _Activity,String wage_Roll,String cover,String wage_turnover_code){
		...
		try{
			CCJ_Rater = OR.getORProperties();
			String t_activity = _Activity.replaceAll("–", "").replaceAll(" ", "_").replaceAll(",", "").replaceAll("__", "_").replaceAll("-", "").replaceAll("__", "_");
			t_activity = "CCJ_"+t_activity + "_"+cover+"_BR_"+wage_turnover_code+"_"+wage_Roll;
			
			//Bio_Fuels_Manual_EL_BR_W_100k
		
			r_value = Double.parseDouble(CCJ_Rater.getProperty(t_activity));
	...
	public double get_Book_Rate_from_Properties_MONEY(String _Activity,int initial_rate){
		...
		try{
			...
			if(_Activity.contains("Any other loss"))
				...
				if(initial_rate > Integer.parseInt(CCJ_Rater.getProperty("Limit_of_cash_in_locked_safe_MONEY_Default_Value")))
					t_activity = "Cash_In_Safe/Transit_in_excess_of_1000_MONEY_BR";
				else
					return 0.0;
			}
	...
	public double get_highest_MD_Buildings_Occupancy_Rate_(){
		...
		try{
			...
			if(common_CCJ.MD_Building_Occupancies_list.size() > 0){
				for(String md_occupancy : common_CCJ.MD_Building_Occupancies_list){
	...
	public double get_BI_Book_Rate(String bi_Activity){
		...
		if(is_BI_Additional_Extensions("BI", bi_Activity)) {//For BI - Additional Extensions section
			...
			common_CCJ.Book_rate_Rater_output.put("BI"+"_"+bi_Activity, book_rate);
			return 0.0;
		}
		//Basic Business Interruption
		...
		{ 
			...
			book_rate = Double.parseDouble(CCJ_Rater.getProperty(t_activity));
		...
		return book_rate;
			...
			public boolean is_EL_Additional_Cover(String cover,String el_Activity)
			{
				...
				CCJ_Rater = OR.getORProperties();
				int f=0;
				String el_add_cover = CCJ_Rater.getProperty("CCJ_EL_AdditionalCovers_List");
			...
			public boolean is_EL_No_Rate_Activity_Refer(String cover,String el_Activity)
			{
				...
				CCJ_Rater = OR.getORProperties();
				int f=0;
				String el_add_cover = CCJ_Rater.getProperty("CCJ_EL_Refer_No_Rate_Activities");
				String[] list_cov = el_add_cover.split(":");
			...
			public boolean is_PL_Refer_Activity(String cover,String pl_Activity)
			{
				...
				CCJ_Rater = OR.getORProperties();
				int f=0;
				String pl_refer = CCJ_Rater.getProperty("CCJ_PL_Refer_No_Rate_Activities");
	...
	public double get_EL_Book_Rate(String el_Activity){
		...
		common_CCJ.Book_rate_Rater_output.put("EL"+"_"+el_Activity, book_rate);
		return book_rate;
	...
	public double get_PL_Book_Rate(String pl_Activity,String category){
		...
		if(category.contains("BF")){
			...
			pl_bonafide_cent = Double.parseDouble(CCJ_Rater.getProperty("CCJ_PL_Bonafide_Cent_Rate"));
		...
		common_CCJ.Book_rate_Rater_output.put("PL"+"_"+pl_Activity, book_rate);
		return book_rate;
	...
	public double get_PAS_Book_Rate(String pas_Activity){
		...
		try{
			...
			common_CCJ.Book_rate_Rater_output.put("PAS"+"_"+pas_Activity, book_rate);
			return book_rate;
	...
	public double get_TER_Book_Rate(String ter_Activity){
		...
		try{
			...
			common_CCJ.Book_rate_Rater_output.put("TER_"+ter_Activity, book_rate);
			return book_rate;
	...
	public double get_GIT_Book_Rate(String git_Activity){
		...
		common_CCJ.Book_rate_Rater_output.put("GIT"+"_"+git_Activity, book_rate);
		return book_rate;
	...
	public double get_Money_Book_Rate(String money_Activity,int initial_rate){
		...
		common_CCJ.Book_rate_Rater_output.put("MONEY"+"_"+money_Activity, book_rate);
		return book_rate;
	...
	public String get_WageRoll_Turnover_string(String cover,String pl_Activity){
		...
		CCJ_Rater = OR.getORProperties();
		int f=0;
		String pl_act_1 = CCJ_Rater.getProperty("CCJ_PL_TurnOver_Based_Activitis_1");
		String pl_act_2 = CCJ_Rater.getProperty("CCJ_PL_TurnOver_Based_Activitis_2");
		...
		public boolean is_BI_Additional_Extensions(String cover,String bi_Activity)
		{
			...
			CCJ_Rater = OR.getORProperties();
			int f=0;
			String bi_add_ext = CCJ_Rater.getProperty("BI_Additional_Extensions");
			...
			public boolean is_BI_Extensions(String cover,String bi_Activity)
				{
					...
					CCJ_Rater = OR.getORProperties();
					int f=0;
					String bi_add_ext = CCJ_Rater.getProperty("BI_Extensions");
					String[] list_ext = bi_add_ext.split(":");
	...
	public String get_WageRoll_format(String cover,String el_Activity){
		...
		if(wage_Roll <= 100000)
			...
			_limit_wage = 500000;
			_wageRoll = "Above_"+_limit_wage/1000 + "k";
			return _wageRoll;
		}
	...
	public String get_Turnover_format(String cover,String pl_Activity){
		...
		}else if(turnover_ > 50000000){
			_limit_turnover = 50000000;
			turnover = "Above_"+_limit_turnover/1000 + "k";
			return turnover;
		}
...
/**
* 
* This method Calculates Book Rate for CCJ Autorated Covers [BI, Money, EL , PL, GIT, PAS, TER].
* 
*/
		...
		case "BI": //Business Interruption
			...
			for(String activity : bi_activites){
				if(activity.contains("Contract Sites") || activity.contains("Loss Of Attraction") 
						||  activity.isEmpty()) {
					common_CCJ.Book_rate_Rater_output.put(coverName+"_"+activity, 0.0);
					continue;
				}else{
			...
			//System.out.println(common_CCJ.Book_rate_Rater_output);
			break;
...
public double calculate_LE_Book_Premium(Map<Object,Object> data_map){
	try{
		...
		book_premium = Double.parseDouble(CCJ_Rater.getProperty("CCJ_LE_"+wage_range+"_BP"));
...
public double get_PL_Auto_Adjustment_(){
		...
		try{
			...
			if(PL_Indemnity_limit <= 1000000){
				auto_rate = Double.parseDouble(CCJ_Rater.getProperty("CCJ_PL_LOI_LoadDiscount_1000000"));
			}else if(PL_Indemnity_limit > 1000000 && PL_Indemnity_limit <= 2650000){
				auto_rate = Double.parseDouble(CCJ_Rater.getProperty("CCJ_PL_LOI_LoadDiscount_2650000"));
			}else if(PL_Indemnity_limit > 2650000 && PL_Indemnity_limit <= 3900000){
				auto_rate = Double.parseDouble(CCJ_Rater.getProperty("CCJ_PL_LOI_LoadDiscount_3900000"));
			}else if(PL_Indemnity_limit > 3900000 && PL_Indemnity_limit <= 5200000){
				auto_rate = Double.parseDouble(CCJ_Rater.getProperty("CCJ_PL_LOI_LoadDiscount_5200000"));
			}else if(PL_Indemnity_limit > 5200000 && PL_Indemnity_limit <= 6500000){
				auto_rate = Double.parseDouble(CCJ_Rater.getProperty("CCJ_PL_LOI_LoadDiscount_6500000"));
			}else{
				auto_rate = Double.parseDouble(CCJ_Rater.getProperty("CCJ_PL_LOI_LoadDiscount_Above_6500000"));
			}
		}catch(Throwable t){
...
public double get_BI_Auto_Adjustment_(String bi_Activity, int indemnity_period){
		...
		if(bi_Activity.equalsIgnoreCase("Additional increased costs of working") || bi_Activity.contains("Increased Cost of Working")
			...
			auto_rate = Integer.parseInt(CCJ_Rater.getProperty("IP_discount_BI_"+indemnity_period+"_Months"));
...
public double func_Check_Minimum_Premium(String coverName,String activityName,double bookPremium){
			...
			boolean isPLHeat = is_PL_Heat_Activity(coverName, activityName);
			try{
				
				if(coverName.equalsIgnoreCase("PL") && isPLHeat ){
					_Min_Premium_ = get_Heat_Min_Premium_from_Properties(coverName,activityName);
				}else{
...
public double get_Min_Premium_from_Properties(String coverName,String _Activity){
		...
		try{
			CCJ_Rater = OR.getORProperties();
			String t_activity = _Activity.replaceAll("–", "").replaceAll(" ", "_").replaceAll(",", "").replaceAll("__", "_");
			t_activity = "CCJ_"+t_activity + "_"+coverName+"_MP";
...
public double get_Heat_Min_Premium_from_Properties(String coverName,String _Activity){
	...
	try{
		...
		if(PL_LOI <= 1000000)
			t_activity="CCJ_Heat_IL_1000000";
		else if(PL_LOI > 1000000 && PL_LOI <= 2650000)
			t_activity="CCJ_Heat_IL_2650000";
		else if(PL_LOI > 2650000 && PL_LOI <= 6500000)
			t_activity="CCJ_Heat_IL_6500000";
		...
		case "Material Damage" :
			...
			case "Business Interruption" :
				...
				for(int i = 0; i< totalRows-2; i++){
					...
					if(!s_Description.contains("Business Interruption")){
						...
						c_BookP = Double.parseDouble((String)data_map.get(s_InnerSheetName).get(iBespokeIndex).get(i_abvr+"Premium"));
						c_BookRatePerc  = common_CCJ.Book_rate_Rater_output.get("BI"+"_"+s_Activity);						
			...
			case "Money & Assault" :
				...
				for(int i = 0; i< totalRows-2; i++){
					...
					c_BookRatePerc  = common_CCJ.Book_rate_Rater_output.get("MONEY"+"_"+s_Description);				
					c_BookP = ((c_SumIns * c_BookRatePerc)/100);
			...
			case "Employers Liability" :
				...
				for(int i = 0; i< totalRows-2; i++){
					...
					if(!s_Description.contains("Employers Liability")){
						...
						c_BookP = Double.parseDouble((String)data_map.get(s_InnerSheetName).get(iBespokeIndex).get(i_abvr+"Premium"));
						c_BookRatePerc  = common_CCJ.Book_rate_Rater_output.get("EL"+"_"+s_Activity);						
						c_TechAdjust = Double.parseDouble((String)data_map.get(s_InnerSheetName).get(iBespokeIndex).get(i_abvr+"TechAdjust"));
						c_CommAdjust = Double.parseDouble((String)data_map.get(s_InnerSheetName).get(iBespokeIndex).get(i_abvr+"CommAdjust"));
						c_OverrideP = Double.parseDouble((String)data_map.get(s_InnerSheetName).get(iBespokeIndex).get(i_abvr+"PremiumOverride"));
						...
						c_minP = common_CCJ.get_Min_Premium_from_Properties("EL", s_Activity);
			...
			case "Public Liability" :
				...
				for(int i = 0; i< totalRows-2; i++){
					...
					if(!s_Description.contains("Public Liability")){
						...
						c_BookP = Double.parseDouble((String)data_map.get(s_InnerSheetName).get(iBespokeIndex).get(i_abvr+"Premium"));
						c_BookRatePerc  = common_CCJ.Book_rate_Rater_output.get("PL"+"_"+s_Activity);						
						c_BookRate = Double.parseDouble((String)data_map.get(s_InnerSheetName).get(iBespokeIndex).get(i_abvr+"Premium"));
						c_TechAdjust = Double.parseDouble((String)data_map.get(s_InnerSheetName).get(iBespokeIndex).get(i_abvr+"TechAdjust"));
						c_CommAdjust = Double.parseDouble((String)data_map.get(s_InnerSheetName).get(iBespokeIndex).get(i_abvr+"CommAdjust"));
						c_OverrideP = Double.parseDouble((String)data_map.get(s_InnerSheetName).get(iBespokeIndex).get(i_abvr+"PremiumOverride"));
						...
						c_minPm = common_CCJ.get_Min_Premium_from_Properties("PL", s_Activity);
						if(c_BookP < c_minPm){
							c_BookP = c_minPm;
						}	
					...
					c_AutoAdjust  = get_PL_Auto_Adjustment_();
					c_BookRate =  c_BookP + ((c_BookP*c_AutoAdjust)/100);
					c_RevisedP =  c_BookRate + ((c_BookRate*c_TechAdjust)/100);
			...
			case "Personal Accident Standard" :
				...
				for(int i = 0; i< totalRows-2; i++){
					...
					c_CommAdjust = Double.parseDouble((String)map_data.get(i_abvr+"CommAdjust"));
					c_OverrideP = Double.parseDouble((String)map_data.get(i_abvr+"PremiumOverride"));
					c_BookRatePerc  = common_CCJ.Book_rate_Rater_output.get("PAS"+"_"+s_Activity);				
			...
			case "Goods in Transit" :
				...
				for(int i = 0; i< totalRows-2; i++){
					...
					c_CommAdjust = Double.parseDouble((String)map_data.get(i_abvr+"CommAdjust"));
					c_OverrideP = Double.parseDouble((String)map_data.get(i_abvr+"PremiumOverride"));
					c_BookRatePerc  = common_CCJ.Book_rate_Rater_output.get("GIT"+"_"+s_Description);				
					c_BookP = ((c_SumIns * c_BookRatePerc)/100);
			...
			case "Terrorism" :
				...
				for(int i = 0; i< totalRows-2; i++){
					...
					c_BookRatePerc  = common_CCJ.Book_rate_Rater_output.get("TER"+"_"+s_Description);				
					c_BookP = ((c_SumIns * c_BookRatePerc)/100);
			...
			case "Legal Expenses" :
				...
				for(int i = 0; i< totalRows-2; i++){
					...
					c_BookP = common_CCJ.calculate_LE_Book_Premium(map_data);
					c_TechAdjust = Double.parseDouble((String)map_data.get(i_abvr+"TechAdjust"));
					c_CommAdjust = Double.parseDouble((String)map_data.get(i_abvr+"CommAdjust"));
					c_OverrideP = Double.parseDouble((String)map_data.get(i_abvr+"PremiumOverride"));
...
public void MTAFlow(String code,String fileName) throws ErrorInTestMethod{
	...
	try{
		...
		common_CCJ.MD_Building_Occupancies_list.clear();
		common_HHAZ.CoversDetails_data_list.clear();
		common_HHAZ.PremiumFlag = false;
		String navigationBy = CONFIG.getProperty("NavigationBy");
		...
		customAssert.assertTrue(funcClaimsExperience(common.MTA_excel_data_map), "Previous claim function is having issue(S) .");
			...
			if(((String)common.MTA_excel_data_map.get("CD_MaterialDamage")).equals("Yes")){	
				customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Material Damage"),"Issue while Navigating to Material Damage screen.");
				customAssert.assertTrue(MaterialDamagePage(common.MTA_excel_data_map), "Material DamagePage function is having issue(S) . ");
			}	
			...
			if(((String)common.MTA_excel_data_map.get("CD_Money&Assault")).equals("Yes")){	
				customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Money & Assault"),"Issue while Navigating to Money & Assault screen.");
				customAssert.assertTrue(MoneyAssaultPage(common.MTA_excel_data_map), "Material DamagePage function is having issue(S) . ");
			}
			...
			if(((String)common.MTA_excel_data_map.get("CD_PublicLiability")).equals("Yes")){	
				customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Public Liability"),"Issue while Navigating to Public Liability screen.");
				customAssert.assertTrue(PublicLiabilityPage(common.MTA_excel_data_map), "Public Liability function is having issue(S) . ");
			}
			if(((String)common.MTA_excel_data_map.get("CD_PersonalAccidentStandard")).equals("Yes")){	
				customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Personal Accident Standard"),"Issue while Navigating to Personal Accident Standard screen.");
				customAssert.assertTrue(PersonalAccidentStandardPage(common.MTA_excel_data_map), "Personal Accident Standard function is having issue(S) . ");
			}
			/*if(((String)common.MTA_excel_data_map.get("CD_PersonalAccidentOptional")).equals("Yes")){	
				customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Personal Accident Optional"),"Issue while Navigating to Personal Accident Optional screen.");
				customAssert.assertTrue(PersonalAccidentOptionalPage(common.MTA_excel_data_map), "Personal Accident Optional function is having issue(S) . ");
			}*/
			if(((String)common.MTA_excel_data_map.get("CD_DeteriorationofStock")).equals("Yes")){	
				customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Deterioration of Stock"),"Issue while Navigating to Deterioration of Stock screen.");
				customAssert.assertTrue(DeteriorationofStockPage(common.MTA_excel_data_map), "Deterioration of Stock function is having issue(S) . ");
			}
			if(((String)common.MTA_excel_data_map.get("CD_GoodsInTransit")).equals("Yes")){	
				customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Goods in Transit"),"Issue while Navigating to Goods in Transit screen.");
				customAssert.assertTrue(GoodsinTransitPage(common.MTA_excel_data_map), "Goods in Transit function is having issue(S) . ");
			}
			if(((String)common.MTA_excel_data_map.get("CD_LegalExpenses")).equals("Yes")){	
				customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Legal Expenses"),"Issue while Navigating to Legal Expenses screen.");
				customAssert.assertTrue(LegalExpensesPage(common.MTA_excel_data_map), "Legal Expenses function is having issue(S)  . ");
			}
...
public void CancellationFlow(String code,String event) throws ErrorInTestMethod{
	...
	try{
		...
		customAssert.assertTrue(Cancel_PremiumSummary(common.CAN_excel_data_map), "Issue in verifying Premium summary for cancellation");
...
public boolean is_BI_referral_activity(String bi_Activity)
{
	...
	CCJ_Rater = OR.getORProperties();
	int f=0;
	String bi_act_list = CCJ_Rater.getProperty("CCD_BI_Referral_Activities");
...
public boolean is_EL_referral_activity(String el_Activity)
{
	...
	CCJ_Rater = OR.getORProperties();
	int f=0;
	String el_act_list = CCJ_Rater.getProperty("CCD_EL_Referral_Activities");
...
public boolean is_PL_referral_activity(String pl_Activity)
{
	...
	CCJ_Rater = OR.getORProperties();
	int f=0;
	String pl_act_list = CCJ_Rater.getProperty("CCD_PL_Referral_Activities");
...
public boolean is_Trade_referral_activity(String trade_Activity)
{
	...
	CCJ_Rater = OR.getORProperties();
	int f=0;
	String pl_act_list = CCJ_Rater.getProperty("CCD_Trade_Referral_Activities");
	String[] list_act = pl_act_list.split(":");
...
//For PL Heat Minimum Premium
public boolean is_PL_Heat_Activity(String cover,String pl_Activity)
	{
		
		CCJ_Rater = OR.getORProperties();
		int f=0;
		String pl_ = CCJ_Rater.getProperty("CCJ_PL_HeatMinimum_Activities");
		String[] pl_heat = pl_.split(":");
		
		for(String bi_ext : pl_heat){
			if(bi_ext.equalsIgnoreCase(pl_Activity))
			{
				f=1;
				return true;
...
public boolean renewalSearchPolicyNEW(Map<Object,Object> map_data){
	...
	if(rows.size()>0){
		...
		String assigned_To = SearchedPolicyTable.findElement(By.xpath("//tbody//tr[1]//td[9]")).getText();
		...
		TestUtil.WriteDataToXl(CommonFunction.product+"_"+CommonFunction.businessEvent, "Policy General",(String)common.Renewal_excel_data_map.get("Automation Key"), "PG_ProposerName", praposerName, common.Renewal_excel_data_map);
		...
		if(assigned_To!=null && !assigned_To.isEmpty()){
			common_HHAZ.isAssignedToUW=true;
		}
		...
		TestUtil.reportStatus("Policy: "+PolicyNumber+" successfully searched . ", "Info", true);
		return retvalue;
	}else{
