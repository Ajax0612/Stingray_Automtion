...
public class CommonFunction_XOC extends TestBase{
...
public void NewBusinessFlow(String code,String event) throws ErrorInTestMethod{
	...
	try{
		...
		customAssert.assertTrue(common_HHAZ.funcEndorsementOperations(common.NB_excel_data_map),"Endorsement is having issue(S).");
...
public void MTAFlow(String code,String fileName) throws ErrorInTestMethod{
	...
	try{
		...
		try{
				...
				customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Endorsements"),"Issue while Navigating to Property Owners Liability screen.");
//				customAssert.assertTrue(common_HHAZ.funcEndorsementOperations(common.MTA_excel_data_map),"Endorsement is having issue(S).");
...
public void RewindFlow(String code,String event) throws ErrorInTestMethod{
	...
	try{
		...
		if(!common.currentRunningFlow.equalsIgnoreCase("Renewal") && !TestBase.businessEvent.equals("MTA")){
			NewBusinessFlow(code,"NB");
		}
...
public void RenewalFlow(String code,String event) throws ErrorInTestMethod{
	...
	try{
		...
		customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Endorsements"),"Issue while Navigating to Property Owners Liability screen.");
//		customAssert.assertTrue(common_HHAZ.funcEndorsementOperations(common.Renewal_excel_data_map),"Endorsement is having issue(S).");
			 ...
			 if(elements.get(i).isDisplayed()){
				 if(elements.get(i).getAttribute("name").contains("_proposer_")){
					 ...
					 try{
						 ...
						 q_value = (String)data_map.get(sInput);
						 sel.selectByVisibleText(q_value);
					 }
					 catch(Throwable t){
						 //sel.selectByVisibleText("No");
...
//		 xoc_details_
		 if(k.isDisplayedField("XOC_MFD_ProductLiability")){
			 ...
			 String[] MDF_Limits = ((String)data_map.get("XOC_MFD_ProductLiability")).split(",");
...
public boolean funcPolicyGeneral(Map<Object, Object> map_data, String code, String event) {
	...
	try{
		...
		String[] geographical_Limits = ((String)data_map.get("PG_GeoLimit")).split(",");
		List<WebElement> ul_elements = driver.findElements(By.xpath("//ul"));
		...
		String[] Prof_Bodies = ((String)data_map.get("PG_InsuredDetails_Q1")).split(",");
		...
		// Select Trade Code :
			...
			if(!sValue.equalsIgnoreCase("") || sValue!=null){
				customAssert.assertTrue(XOC_SelectTradeCode((String)data_map.get("PG_TCS_TradeCode") , "Policy General" , 0),"Trade code selection function is having issue(S).");
			}
...
public boolean funcPolicyGeneral_MTA(Map<Object, Object> map_data, String code, String event) {
	...
	try{
		...
		// Select Trade Code :
			...
			/*String sValue = (String)map_data.get("PG_TCS_TradeCode");
			if(!sValue.equalsIgnoreCase("") || sValue!=null){
				customAssert.assertTrue(XOC_SelectTradeCode((String)common.MTA_excel_data_map.get("PG_TCS_TradeCode") , "Policy General" , 0),"Trade code selection function is having issue(S).");
			}*/
...
public boolean func_GRD_AddMainP(Map<Object, Object> map_data, String s_Premises){
	...
	try{
		...
		case "Renewal":
			...
			if(!TestBase.product.contains("COA") && !common.currentRunningFlow.contains("Renewal")){
				for(int i = 0; i < length; i++){
					customAssert.assertTrue(k.Click("COB_GRD_"+s_Section),"Unable to click on GRDAddMainPremises button");
					switch (internal_data_map.get(s_Sheet).get(i).get(s_Section+"_CopyAddress")) {
					case "Yes":
					...
					break;
					default:
						customAssert.assertTrue(k.Input("XOC_GRD_Address1", internal_data_map.get(s_Sheet).get(i).get(s_Section+"_AddressLine1")),"Unable to enter value in Address line 1");
						customAssert.assertTrue(k.Input("XOC_GRD_Address2", internal_data_map.get(s_Sheet).get(i).get(s_Section+"_AddressLine2")),"Unable to enter value in Address line 2");
						customAssert.assertTrue(k.Input("XOC_GRD_Address3", internal_data_map.get(s_Sheet).get(i).get(s_Section+"_AddressLine3")),"Unable to enter value in Address line 3");
						customAssert.assertTrue(k.Input("XOC_GRD_Address4", internal_data_map.get(s_Sheet).get(i).get(s_Section+"_AddressLine4")),"Unable to enter value in Address line 4");
						customAssert.assertTrue(k.Input("XOC_GRD_Town", internal_data_map.get(s_Sheet).get(i).get(s_Section+"_Town")),"Unable to enter value in Town field");
						customAssert.assertTrue(k.Input("XOC_GRD_PostCode", internal_data_map.get(s_Sheet).get(i).get(s_Section+"_PostCode")),"Unable to enter value in Postcode field");
						customAssert.assertTrue(k.Input("XOC_GRD_County", internal_data_map.get(s_Sheet).get(i).get(s_Section+"_County")),"Unable to enter value in County field");
						customAssert.assertTrue(k.DropDownSelection("XOC_GRD_Country", internal_data_map.get(s_Sheet).get(i).get(s_Section+"_Country")),"Unable to enter value in Country field");
						break;
					}
...
public boolean ThirdPartyMotorLiability(Map<Object, Object> map_data){
	try{
		...
		//customAssert.assertTrue(!k.getListAttributeIsEmpty("XOC_TPML_ExcessLayer", ""),"Primary Layer Insurance Cover Field Should Contain Valid Name  .");
		//Product Details
		String[] Vehicle_type = ((String)map_data.get("VehicleType")).split(",");
...
public boolean funcAddBespoke(Map<Object, Object> map_data, String s_Bespoke,  String s_CoverName){
	...
	try{
			...
			for(int i = 0; i < AllBespoke.length; i++){
				...
				k.scrollInViewByXpath("//*[contains(@id,'bespoke-item')]");
				k.waitFiveSeconds();
				driver.findElement(By.xpath("//*[contains(@id,'bespoke-item')]")).click();
								
				/*if(!driver.findElement(By.xpath(OR.getlocatorValue("XOC_TPML_bespoke_desc"))).isDisplayed()){
					k.scrollInViewWithOutError("XOC_CL_AddBeSpoke");
					WebDriverWait wait = new WebDriverWait(driver, 10);
					wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(".//*[@id='add-bespoke-item']")));
				...
				switch(s_CoverName){
					case "JCT" :
						...
						if(k.isDisplayed("XOC_TPML_bespoke_desc")){
							customAssert.assertTrue(k.Input("XOC_TPML_bespoke_desc",(String)internal_data_map.get(innerSheetName).get(i).get(s_Abvr+"_Description")),"Unable to put value in Description field");
						}
						if(k.isDisplayed("XOC_TPML_bespoke_amt")){
							customAssert.assertTrue(k.Input("XOC_TPML_bespoke_amt",(String)internal_data_map.get(innerSheetName).get(i).get(s_Abvr+"_BookPremium")),"Unable to put value in BookPremium field");
						}
						break;
					case "PPL" :	
					...
					default :	
						...
						if(k.isDisplayed("COB_AddB_LOI")){
							customAssert.assertTrue(k.Input("COB_AddB_LOI",(String)internal_data_map.get(innerSheetName).get(i).get(s_Abvr+"_ProvideFullDetails")),"Unable to put value in PleaseProvideFullDetails field");
						}
			...
			for(int i = 0; i< totalRows-2; i++){
				...
				if(i == 0){
					...
					if((s_Section.contains("Public & Products Liability") || s_Section.contains("Employers Liability")) && s_CoverName.contains("Combined Liability")){
						...
						customAssert.assertTrue(k.DynamicXpathWebDriver_Inner(driver, s_TablePath + "/tbody/tr["+(i+1)+"]/td[3]/input", internal_data_map, s_InnerSheetName, bs_counter, i_abvr, "TechAdjust", "TechAdjust", "Input"),"Unable to enter TechAdjust in table");
						customAssert.assertTrue(k.DynamicXpathWebDriver_Inner(driver, s_TablePath + "/tbody/tr["+(i+1)+"]/td[4]/input", internal_data_map, s_InnerSheetName, bs_counter, i_abvr, "CommAdjust", "CommAdjust", "Input"),"Unable to enter CommAdjust in table");
				...
				//Calculation :
					if(i == 0){
						...
						if((s_Section.contains("Public & Products Liability") || s_Section.contains("Employers Liability") || s_Section.contains("Third Party Motor Liability")) && (s_CoverName.contains("Combined Liability")||s_CoverName.contains("Public & Products Liability"))){
							...
							c_BookP = Double.parseDouble((String)internal_data_map.get(s_InnerSheetName).get(bs_counter).get(i_abvr+"BookPremium"));
							c_TechAdjust = Double.parseDouble((String)internal_data_map.get(s_InnerSheetName).get(bs_counter).get(i_abvr+"TechAdjust"));
							c_commAdjust = Double.parseDouble((String)internal_data_map.get(s_InnerSheetName).get(bs_counter).get(i_abvr+"CommAdjust"));
							bs_counter = bs_counter+1; 
						} 
					}
...
public boolean CombinedLiability(Map<Object, Object> map_data){
	try{
		...
		String[] EL_InsuranceCover_Limits = ((String)map_data.get("XOC_CL_LayerInsuranceCoverList")).split(",");
		...
		String[] PL_InsuranceCover_Limits = ((String)map_data.get("XOC_CL_PL_LayerInsuranceCoverList")).split(",");
		...
		PL_Product_Limits = ((String)map_data.get("XOC_CL_PL_ProductLibilityRskMgmtFtrs")).split(",");
	...
	if(!common.currentRunningFlow.equals("MTA")){
		...
		if(((String)map_data.get("XOC_CL_TPMPL_OptionalExtn")).equalsIgnoreCase("Yes")){
			...
			String[] TPML_InsuranceCover_Limits = ((String)map_data.get("XOC_CL_TPMPL_LayerInsuranceCoverList")).split(",");
...
//			}
			...
			String[] InsuranceCover_Limits = ((String)map_data.get("XOC_CL_TPMPL_VehicleType")).split(",");
			List<WebElement> ul_elements = driver.findElements(By.xpath("//ul"));
			for(String ins_limit : InsuranceCover_Limits){
				for(WebElement each_ul : ul_elements){
					k.pressDownKeyonPage();
		...
		// Adding bespoke items
			String coverName = null;
			String Sval = (String) map_data.get("CL_AddBeSpokeItems");
			String bsAdd= (String)map_data.get("CL_addBespoke?");  
			if(Sval!=null && bsAdd.contains("Yes")){
...
public boolean PublicProductsLiabilityPage(Map<Object, Object> map_data){
	...
	try{
		...
		if(!common.currentRunningFlow.equalsIgnoreCase("NB")){
	    	  ...
	    	  customAssert.assertTrue(common_HHAZ.deleteItems(),"Delete Items function is having issues.");
	  //  	  customAssert.assertTrue(removeDropDownValues(),"Delete Items function is having issues.");
		...
		String[] PL_InsuranceCover_Limits = ((String)map_data.get("XOC_PL_LyerInsuranceCoverList")).split(",");
...
//		customAssert.assertTrue(!k.getAttributeIsEmpty("XOC_CL_PL_ProductDesc", "value"),"Public Liability Primary Layer of Indeminity Field should not be empty  .");
		String[] PL_Product_Limits = null;
		k.waitFiveSeconds();
		PL_Product_Limits = ((String)map_data.get("XOC_PL_ProductLiabilityRskMgmtFtrs")).split(",");
		//List<WebElement> prod_ul_elements = driver.findElements(By.xpath("//ul"));
		String item1 = "//*[text()='"; // Batch recording']
		String l_item = null;
		for(String ins_limit : PL_Product_Limits){
		...
		if(((String)map_data.get("XOC_PL_TPMLExtns")).equalsIgnoreCase("Yes")){
			...
			String[] TPML_InsuranceCover_Limits = ((String)map_data.get("XOC_PL_TPML_LayerInsuranceCovelist")).split(",");
			...
			for(String ins_limit : TPML_InsuranceCover_Limits){
				for(WebElement each_ul : tpml_ul_elements){
					...
					if(each_ul.findElement(By.xpath("//li[text()='"+ins_limit+"']")).isDisplayed())
						...
						if(!TestBase.product.equals("XOC"))
						customAssert.assertTrue(k.Click("XOC_ELPL_TPMLCoverlist"),"Error while Clicking PL Primary Layer Insurance Cover List object . ");
			...
			if(common.currentRunningFlow.equals("NB")|| TestBase.businessEvent.equals("Rewind")||common.currentRunningFlow.equals("Renewal")||common.currentRunningFlow.equals("Requote")){
			k.waitTwoSeconds();
			
			if(!TestBase.product.equals("XOC"))
			driver.findElement(By.xpath("//*[@id='mainpanel']")).click();
			
			}else if(TestBase.businessEvent.equals("MTA")&&common.NB_excel_data_map.get("CD_Public&ProductsLiability").equals("No")){
				
				driver.findElement(By.xpath("//*[@id='mainpanel']")).click();
			}
			
			String[] InsuranceCover_Limits = ((String)map_data.get("XOC_PL_TPMLVehicleType")).split(",");
			List<WebElement> ul_elements = driver.findElements(By.xpath("//ul"));
			for(String ins_limit : InsuranceCover_Limits){
				for(WebElement each_ul : ul_elements){
				    ...
				    k.waitTwoSeconds();
					if(each_ul.findElement(By.xpath("//li[text()='"+ins_limit+"']")).isDisplayed()){
						if(!(common.currentRunningFlow.equals("NB")||common.currentRunningFlow.equals("Renewal")||common.currentRunningFlow.equals("MTA")||common.currentRunningFlow.equals("Rewind")||common.currentRunningFlow.equals("Requote"))){
			...
			for(int i=0;i<aa.length;i++){
				k.scrollInViewByXpath("//input[@name='"+aa[i]+"']");
...
//			
		...
		// Adding bespoke items
			...
			String Sval = (String)map_data.get("PPL_AddBeSpokeItems");
			String bsAdd= (String) map_data.get("PPL_addBespoke?");  
			if(Sval!=null && bsAdd.contains("Yes")){
...
public boolean JCTPage(Map<Object, Object> map_data){
	...
	try{
		...
		String[] InsuranceCover_Limits = ((String)map_data.get("XOC_JCT_LyerInsuranceCoverList")).split(",");
		List<WebElement> ul_elements = driver.findElements(By.xpath("//ul"));
		for(String ins_limit : InsuranceCover_Limits){
			for(WebElement each_ul : ul_elements){
		...
		// Adding bespoke items
			String coverName1 = null;
			String Sval = (String) map_data.get("JCT_AddBeSpokeItems");
			String bsAdd= (String) map_data.get("JCT_addBespoke?");  
			if(Sval!=null && bsAdd.contains("Yes")){
								...
								j_exe.executeScript("arguments[0].scrollIntoView(true);", driver.findElement(By.xpath("//*[contains(@name,'"+c_locator+"')]")));
									...
									if(((String) common.MTA_excel_data_map.get("CD_"+coverName)).equalsIgnoreCase("No"))
										continue;
							 			...
							 			customAssert.assertTrue(common_XOC.selectCover(coverWithLocator,map_data), "Select covers function is having issue(S) . ");
										common_VELA.CoversDetails_data_list.add(coverWithoutLocator);
									...
									if(((String) common.MTA_excel_data_map.get("CD_"+coverName)).equalsIgnoreCase("Yes"))
										...
										customAssert.assertTrue(common_XOC.deSelectCovers(coverWithLocator,map_data), "Select covers function is having issue(S) . ");
								 }
...
public boolean selectCover(String coverNameWithLocator,Map<Object, Object> map_data){
	...
	try{
			...
			if(common.product.equalsIgnoreCase("XOC")&&common.currentRunningFlow.equalsIgnoreCase("MTA")){
				c_locator = coverNameWithLocator.split("__")[1];
			}
			k.waitTwoSeconds();
...
public boolean funcRewindOperation_MTA(){
	...
	String navigationBy = CONFIG.getProperty("NavigationBy");
	common_VELA.isMTARewindFlow = true;
	common_VELA.isMTARewindStarted=true;
	try{
		...
		if(((String)common.MTA_excel_data_map.get("CD_Add_Remove_Cover")).equalsIgnoreCase("Yes")){
			...
			customAssert.assertTrue(common.funcMenuSelection("Navigate","Premium Summary"),"Issue while Navigating to Premium Summary screen . ");
			customAssert.assertTrue(common_VELA.funcPremiumSummary_MTA(common.MTA_excel_data_map,CommonFunction.product,CommonFunction.businessEvent), "Premium Summary function is having issue(S) . ");
...
public boolean removeDropDownValues(){
	boolean isNotStale=true;
		k.ImplicitWaitOff();
		while(isNotStale){
			try{
				
			List<WebElement> delete_Btns = driver.findElements(By.xpath(".//td[text() = 'Optional Extension']/following::*[@class='select2-selection__choice__remove']"));
				
				for(WebElement element: delete_Btns){
					if(element.isDisplayed())
						element = driver.findElement(By.xpath(".//td[text() = 'Optional Extension']/following::*[@class='select2-selection__choice__remove']"));
						JavascriptExecutor j_exe = (JavascriptExecutor) driver;
