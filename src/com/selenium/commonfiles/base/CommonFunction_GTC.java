	...
	try{
		...
		if(((String)map_data.get("CAR_StorageReq")).equalsIgnoreCase("Yes")){
			...
			if(loc.length() > 0 ){
				...
				WebElement carLoc_Table = null;
				
				carLoc_xpath = "//table[@id='table_require_storage']";
				carLoc_Table = driver.findElement(By.xpath(carLoc_xpath));
				
				Thread.sleep(1000);
	    		List<WebElement> carLoc_colms = carLoc_Table.findElements(By.tagName("th"));
	    		int carLoc_tble_Rows = carLoc_Table.findElements(By.tagName("tr")).size();
	    		
	    		/*if(common.currentRunningFlow.contains("Requote")){
	    			k.ScrollInVewWebElement(carLoc_Table);
	    		}else{
	    			k.scrollInViewWithOutError(carLoc_xpath);
	    		}*/
