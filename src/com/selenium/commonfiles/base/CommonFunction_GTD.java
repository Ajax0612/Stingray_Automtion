...
public class CommonFunction_GTD extends TestBase{
...
public boolean GIT(Map<Object, Object> map_data){
	...
	try{
		...
		customAssert.assertTrue(k.DropDownSelection("GTD_GIT_GeoLimit", (String)map_data.get("GIT_GeoLimit")), "Unable to enter value in GIT_GeoLimit field");
		...
		customAssert.assertTrue(k.DropDownSelection("GTD_GIT_PMovers", (String)map_data.get("GIT_PrimeMovers")), "Unable to enter value in GIT_PrimeMovers field");
		customAssert.assertTrue(k.DropDownSelection("GTD_GIT_DetCover", (String)map_data.get("GIT_CoverForDet")), "Unable to enter value in GIT_CoverForDet field");
		customAssert.assertTrue(k.DropDownSelection("GTD_GIT_DamageCover", (String)map_data.get("GIT_CoverForDamage")), "Unable to enter value in GIT_CoverForDamage field");
		customAssert.assertTrue(k.DropDownSelection("GTD_GIT_WHCover", (String)map_data.get("GIT_Whouse")), "Unable to enter value in GIT_Whouse field");
		...
		String tcd = (String)map_data.get("GIT_TCD");
		...
		String car_AddI = (String)map_data.get("GIT_AddItem");
		if(car_AddI.length() > 0){
			...
			for(int l=0;l<l_Count;l++){
				...
				customAssert.assertTrue(k.Input("GTD_AddItem_DEsc", (String)internal_data_map.get("AddItem").get(l).get("AI_Description")), "Unable to enter Description for AddItem on GIT screen.");
				customAssert.assertTrue(k.Input("GTD_AddItem_Turnover", (String)internal_data_map.get("AddItem").get(l).get("AI_Turnover")), "Unable to enter Turnover for add item on GIT screen.");
				k.clickInnerButton("GTC_inner_page_locator", "Save");
			}
		}
        
        customAssert.assertTrue(k.Click("GTD_CalculatePremium"), "Unable to click on Calculate Premium button.");
		...
		// Input to the premium table :
				...
				for(int i = 0; i< totalRows-2; i++){
				   ...
				   if( s_Description.equals("Flat Premium")){
					...
					String pOverride = (String)internal_data_map.get("AddItem").get(i).get("AI_POverride");
					if(!pOverride.equals("0")){
		...
		// Calculation :
				 ...
				 for(int i = 0; i< totalRows-2; i++){
					 ...
					 //Read values from screen :
					 	...
					 	String s_Turnover = k.GetText_DynamicXpathWebDriver(driver, sTablePath+"/tbody/tr["+(i+1)+"]/td[2]");
					 	...
					 	CommonFunction.compareValues(c_TurnOver ,  Double.parseDouble(s_Turnover) ,"TurnOver for activity  "+s_Description +" of Goods in Transit cover");
					 	CommonFunction.compareValues(c_BookRate ,  Double.parseDouble(s_bookRate) ,"BookRate for activity  "+s_Description +" of Goods in Transit cover");
					 	CommonFunction.compareValues(c_BookP ,  Double.parseDouble(s_bookP) ,"BookP for activity  "+s_Description  +" of Goods in Transit cover");
					 	CommonFunction.compareValues(c_TechAdjust ,  Double.parseDouble(s_TechAdjust) ,"TechAdjust for activity  "+s_Description  +" of Goods in Transit cover");
					 	CommonFunction.compareValues(c_RevisedP ,  Double.parseDouble(s_RevisedP) ,"RevisedP for activity  "+s_Description  +" of Goods in Transit cover");
					 	CommonFunction.compareValues(c_CommAdjust ,  Double.parseDouble(s_CommAdjust) ,"CommAdjust for activity  "+s_Description  +" of Goods in Transit cover");
					 	CommonFunction.compareValues(c_POverride ,  Double.parseDouble(s_pOverride) ,"POverride for activity  "+s_Description  +" of Goods in Transit cover");
					 	CommonFunction.compareValues(c_Premium ,  Double.parseDouble(s_Premium) ,"Premium for activity  "+s_Description  +" of Goods in Transit cover");
					 	
				 }
				 CommonFunction.compareValues(c_TotalP , s_TotalP ,"Total Premium for activity of GIT cover");
				
				 
				 TestUtil.WriteDataToXl(CommonFunction.product+"_"+common.currentRunningFlow, "Premium Summary", (String)map_data.get("Automation Key"), "PS_GoodsinTransit_NetNetPremium", String.valueOf(c_TotalP), map_data);
...
public void RewindFlow(String code,String event) throws ErrorInTestMethod{
	...
	try{
		...
		common_HHAZ.CoversDetails_data_list.add("Goods in Transit");
		customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Premium Summary"),"Issue while Navigating to Premium Summary screen . ");
		...
		customAssert.assertTrue(func_Referrals_MaterialFactsDeclerationPage(), "Referrals for MaterialFactsDeclerationPage function is having issue(S) . ");
		customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Goods in Transit"),"Issue while Navigating to Goods in Transit screen.");
		customAssert.assertTrue(GIT(common.Rewind_excel_data_map), "Goods in Transit function is having issue(S) . ");
		customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Endorsements"),"Issue while Navigating to Endorsements screen.");
...
public void RenewalFlow(String code,String event) throws ErrorInTestMethod{
	...
	try{
		...
		customAssert.assertTrue(common.funcNextNavigateDecesionMaker(navigationBy,"Goods in Transit"),"Issue while Navigating to Cargo screen.");
		customAssert.assertTrue(GIT(common.Renewal_excel_data_map), "Goods in Transit function is having issue(S) . ");
...
public void MTAFlow(String code,String event) throws ErrorInTestMethod{
	...
	try{
			if (((String) common.MTA_excel_data_map.get("MTA_ExistingPolicy")).equalsIgnoreCase("Yes")) {
				common_EP.ExistingPolicyAlgorithm(common.MTA_excel_data_map,
						(String) common.MTA_excel_data_map.get("MTA_ExistingPolicy_Type"),
						(String) common.MTA_excel_data_map.get("MTA_ExistingPolicy_Status"));
			} else {
				if (!common.currentRunningFlow.equalsIgnoreCase("Renewal")) {
					NewBusinessFlow(code, "NB");
				}
